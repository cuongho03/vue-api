...
function updateProductPrices({ product, rate, sourcePriceInclTax = false, deprecatedPriceFieldsSupport = false, finalPriceInclTax = true }) {
    ...
    const hasOriginalPrices = (product.hasOwnProperty('original_price') &&
    ...
    if (!hasOriginalPrices) {
        assignPrice(Object.assign(Object.assign({ product, target: 'original_price' }, priceWithTax), { deprecatedPriceFieldsSupport }));
    ...
    assignPrice(Object.assign(Object.assign({ product, target: 'price' }, priceWithTax), { deprecatedPriceFieldsSupport }));
    if (specialPriceWithTax.price) {
        assignPrice(Object.assign(Object.assign({ product, target: 'special_price' }, specialPriceWithTax), { deprecatedPriceFieldsSupport }));
    }
    if (finalPriceWithTax.price) {
        assignPrice(Object.assign(Object.assign({ product, target: 'final_price' }, finalPriceWithTax), { deprecatedPriceFieldsSupport }));
    }
    if (product.final_price) {
        if (product.final_price < product.price) { // compare the prices with the product final price if provided; final prices is used in case of active catalog promo rules for example
            assignPrice(Object.assign(Object.assign({ product, target: 'price' }, finalPriceWithTax), { deprecatedPriceFieldsSupport }));
            if (product.special_price && product.final_price < product.special_price) { // for VS - special_price is any price lowered than regular price (`price`); in Magento there is a separate mechanism for setting the `special_prices`
                assignPrice(Object.assign(Object.assign({ product, target: 'price' }, specialPriceWithTax), { deprecatedPriceFieldsSupport })); // if the `final_price` is lower than the original `special_price` - it means some catalog rules were applied over it
                assignPrice(Object.assign(Object.assign({ product, target: 'special_price' }, finalPriceWithTax), { deprecatedPriceFieldsSupport }));
            }
            else {
                assignPrice(Object.assign(Object.assign({ product, target: 'price' }, finalPriceWithTax), { deprecatedPriceFieldsSupport }));
            }
    ...
    if (product.special_price && (product.special_price < product.original_price)) {
        if (!isSpecialPriceActive(product.special_from_date, product.special_to_date)) {
            ...
            assignPrice(Object.assign(Object.assign({ product, target: 'price' }, specialPriceWithTax), { deprecatedPriceFieldsSupport }));
        }
    }
    else {
...
exports.checkIfTaxWithUserGroupIsActive = checkIfTaxWithUserGroupIsActive;
...
exports.getUserGroupIdToUse = getUserGroupIdToUse;
